...
function getBuddyListFriends(list: BuddyList): Friend[] {
    return list.members.reduce((friendsArray: Friend[], buddy: Buddy) => {
      // Check if the buddy is an instance of Friend
      if ('phone' in buddy) {
        // Push the buddy to the friendsArray
        friendsArray.push(buddy as Friend);
      }
      return friendsArray;
    }, [] as Friend[]); // Initialize the accumulator as an empty array
  }
  
  // Test for getBuddyListFriends
  const footballBuddies = getBuddyListFriends(myFootballBuddies);
  const bandBuddies = getBuddyListFriends(myBandBuddies);
  
  console.log("Football Buddies:", footballBuddies);
  console.log("Band Buddies:", bandBuddies);